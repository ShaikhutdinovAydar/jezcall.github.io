...
featuresBlock.forEach((card) => {
  ...
  contentOfThing.addEventListener('click', () => {
    ...
    // description
    const description = contentOfThing.querySelector('h2').textContent;
    const remove_description = document.querySelector('.span_content strong');
    if (description != ''){
      remove_description.textContent = description;
    }
    else {
      remove_description.textContent = 'No information about this item';
    }
    ...
    const checkout_now = document.querySelector('.id__block .checkout_now');
    checkout_now.addEventListener('click', () => {
      alert('gg');
    });
